...
struct RootView: View {
                ...
                ForEach(browser.tabs) { tab in
                    Group {
                        if tab.isHome {
                            ...
                            WebContainer(tab: tab, onPull: { tab.reload() }).ignoresSafeArea(edges: .bottom)
                        }
                    }
                    .opacity(tab.id == browser.currentID ? 1 : 0)
                    .scaleEffect(tab.id == browser.currentID ? 1 : 0.98)
                    .allowsHitTesting(tab.id == browser.currentID && !browser.reportMode)
                }
            }
            .padding(.bottom, 92)
    ...
    private var addressBar: some View {
        HStack(spacing: 10) {
            ...
            if addressFocused {
                ...
                IconButton(symbol: "xmark", size: 14) { browser.current?.stop() }.frame(width: 28, height: 28)
            } else if !(browser.current?.isHome ?? true) {
                IconButton(symbol: "arrow.clockwise", size: 15) { browser.current?.reload() }.frame(width: 28, height: 28)
            }
        }
    ...
    private var toolbar: some View {
        HStack {
            IconButton(symbol: "chevron.left", enabled: browser.current?.canGoBack ?? false) { browser.current?.goBack() }
            Spacer()
            IconButton(symbol: "chevron.right", enabled: browser.current?.canGoForward ?? false) { browser.current?.goForward() }
            Spacer()
            ...
            Button { Haptics.tap(); browser.current?.captureSnapshot(); browser.showTabsOverview = true } label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 7).strokeBorder(AeroColor.textPrimary, lineWidth: 2).frame(width: 24, height: 24)
                    Text("\(browser.tabs.count)").font(.system(size: 12, weight: .bold, design: .rounded)).foregroundStyle(AeroColor.textPrimary)
                }
