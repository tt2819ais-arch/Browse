...
struct BookmarksView: View {
    ...
    private func row(title: String, url: String) -> some View {
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 10).fill(VoidColor.accentSoft).frame(width: 38, height: 38)
                Image(systemName: "globe").font(.system(size: 16)).foregroundStyle(VoidColor.accent)
            }
